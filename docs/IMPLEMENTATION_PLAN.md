...
- [ ] **2.4 SSE chat endpoint.** `POST /api/v1/chat` via `sse-starlette`: events `status` (routing/retrieving), `token`, `citation`, `done` (message_id, confidence), `error`. Heartbeat ~15s, `X-Accel-Buffering: no`, graceful `asyncio.CancelledError` on disconnect. Persists the full exchange.
...
- [ ] **3.1 Auth API.** `POST /auth/signup` (user + org + owner membership), `/auth/login`, `/auth/refresh`, `/auth/me`. JWT access (15m) + refresh (7d) in httpOnly cookies; passlib/bcrypt hashing.
